...
export async function cacheConfigAssets(config: PortfolioConfig) {
  ...
  if (out.personal?.avatar) {
    out.personal.avatar.src = await cacheExternalAsset(
      out.personal.avatar.src ?? "",
      out.personal.avatar.fallback ?? "",
      "avatars",
    );
    out.personal.avatar.fallback = out.personal.avatar.fallback
      ? await cacheExternalAsset(out.personal.avatar.fallback, "", "avatars")
      : out.personal.avatar.fallback;
  }
  ...
  if (out.resume?.url) {
    out.resume.url = await cacheExternalAsset(
      out.resume.url,
      undefined,
      "resumes",
    );
