...
export default function CVCard({ config, onContactClick }: CVCardProps) {
  ...
  const age =
    personal.yearOfBirth && privacy.exposeAge
      ? new Date().getFullYear() - personal.yearOfBirth
      : undefined;
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-3 text-muted-foreground">
            {skills.map((skill) => (
              <span
                key={skill.id}
                className="px-3 py-1 bg-muted-foreground/10 rounded-2xl text-sm"
              >
            ...
            {experience.map((exp) => (
              <div key={exp.id} className="border-l-2 border-primary pl-4">
                ...
                <p className="text-sm text-muted-foreground">
                  {exp.start?.slice(0, 10)} –{" "}
                  {exp.end ? exp.end.slice(0, 10) : "Present"}
                </p>
          ...
          <div className="space-y-3">
            {projects.map((proj) => (
              <div key={proj.id} className="border-l-2 border-secondary pl-4">
                <p className="font-semibold">
                  {proj.title} ({capitalize(proj.role)})
                </p>
